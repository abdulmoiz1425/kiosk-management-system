...
Saves uploads to:
  NAS (production):  {NAS_MOUNT_PATH}/{kiosk_name}/{YYYY-MM-DD}/{file_type}_{filename}
  Local (dev/fallback): {MEDIA_ROOT}/{kiosk_name}/{YYYY-MM-DD}/{file_type}_{filename}
...
def get_upload_path(kiosk_name: str, file_type: str, filename: str) -> str:
    ...
    Structure: {kiosk_name}/{YYYY-MM-DD}/{file_type}_{filename}
    """
    safe_kiosk = (
        kiosk_name
        .replace(' ', '_')
    ...
    return os.path.join(safe_kiosk, today, f"{file_type}_{filename}")
